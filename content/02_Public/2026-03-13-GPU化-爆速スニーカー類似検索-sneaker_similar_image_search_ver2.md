## 爆速スニーカー類似検索-sneaker_similar_image_search_ver2
#### 以前のsneaker_similar_image searchの問題点: 
- CPUを使っており、非効率
- CPUだからか、画像をfor文で一枚ずつ見ており、時間がかかる
#### 改善策:
- GPUを使用。[Google-Colab](Google-Colab.md)の「編集」→「ノートブックの設定」から、「TP4 GPU」を選択
- [[2026-03-13-GoogleColabでGPUを使うには手動で変える必要有り]]
#### トラブルシューティング:
- `!pip install -q faiss-gpu`では、Google Colabではエラーが出る。
- 下記のように書き換えることで解決。詳しくは**faiss-gpuの仕様**もしくは[[faiss-gpu]]参照
```
